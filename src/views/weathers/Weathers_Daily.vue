...
export default {
  ...
  methods:{
    Get_Weathers(){
      ...
      Stores_Weathers().Weathers_Index({location : data,exclude : 'daily'}).then(res =>{
        this.weathers = res.data.result;
        if (this.weathers.daily.weather_code){
          this.weathers.daily.weather_code.forEach(item => {
...
<template>
  ...
  <template v-else>
    <div v-for="i in 7" class="q-mb-lg animation-fade-in">
      <div class="date-box text-center">
        <strong class="text-grey-9">{{ this.$filters.date_jalali(this.Methods_FormatUnixTimestamp(weathers.daily.time[i-1]),'jYYYY/jM/jD  H:m:s') }}</strong>
      </div>
      <div class="item-box">
        <div class="row">
          <div class="col-7">
            <div class="row">
              ...
              <div class="q-mt-md">
                <strong class="font-19 text-indigo-2">{{Math.round(weathers.daily.temperature_2m_min[i-1])}}</strong> <strong>/</strong> <strong class="font-19 text-red-3">{{Math.round(weathers.daily.temperature_2m_max[i-1])}}</strong>
              </div>
              <div class="col-12">
          ...
          <div class="col-5">
            <div class="q-mt-xs">
              ...
              <strong class="text-white">{{weathers.daily.wind_speed_10m_mean[i-1]	}}</strong>
            ...
            <div class="q-mt-sm">
              ...
              <strong class="text-white">{{weathers.daily.precipitation_probability_mean[i-1]	}}</strong>
